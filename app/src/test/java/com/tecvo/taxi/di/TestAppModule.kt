...
/**
 * Test module that replaces Firebase providers with mocks to prevent
 * "FirebaseApp is not initialized" errors during unit testing.
 * 
 * This module uses @TestInstallIn to replace AppModule during testing,
 * providing mock Firebase instances instead of calling getInstance()
...
object TestAppModule {
    ...
    // ================================
    // NON-FIREBASE PROVIDERS (Same as AppModule)
    // ================================
    ...
    // ================================
    // FIREBASE PROVIDERS (MOCKED FOR TESTING)
    // ================================

    /**
     * Provides a mock FirebaseDatabase instance for testing.
     * This prevents the "FirebaseApp is not initialized" error
     * that would occur if we called FirebaseDatabase.getInstance().
     */
    @Provides
    @Singleton
    fun provideFirebaseDatabase(): FirebaseDatabase {
        return Mockito.mock(FirebaseDatabase::class.java).apply {
            // Configure mock behavior if needed for specific tests
            println("TestAppModule: Providing mock FirebaseDatabase")
        }
    }

    /**
     * Provides a mock FirebaseAuth instance for testing.
     * This prevents the "FirebaseApp is not initialized" error
     * that would occur if we called FirebaseAuth.getInstance().
     */
    @Provides
    @Singleton
    fun provideFirebaseAuth(): FirebaseAuth {
        return Mockito.mock(FirebaseAuth::class.java).apply {
            // Configure mock behavior if needed for specific tests
            println("TestAppModule: Providing mock FirebaseAuth")
        }
    }
}